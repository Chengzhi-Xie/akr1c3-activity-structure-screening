...
# ── Run all (Updated for Master Folder Automation) ────────────────────────
if __name__ == "__main__":
    master_dir = os.getcwd()
    
    # Automatically find all analysis_results folders inside subdirectories
    # (This works on both Mac/Linux "/" and Windows "\")
    target_dirs = glob.glob(os.path.join("*", "analysis_results"))
    
    if not target_dirs:
        print("Error: Could not find any 'analysis_results' subdirectories.")
        print("Make sure this script is in the master protein folder.")
        exit()

    for target in target_dirs:
        # Extract the ligand folder name (e.g., "1ZQ5_Z1276023166") for the console
        ligand_name = target.split(os.sep)[0]
        
        print(f"\n══════════════════════════════════════════════════════════")
        print(f"Processing Ligand: {ligand_name}")
        print(f"══════════════════════════════════════════════════════════")
        
        # 1. Step INTO the specific ligand's analysis folder
        os.chdir(os.path.join(master_dir, target))
        
        # 2. Run the plots (They will now safely find the local .dat files!)
        try:
            plot_rmsd()
            plot_rmsf()
            plot_rog()
            ...
            print(f"  [!] Skipping a plot — missing data: {e}")
        except Exception as e:
            print(f"  [!] Error generating plot: {e}")
            
        # 3. Step BACK to the master folder to prepare for the next ligand
        os.chdir(master_dir)
